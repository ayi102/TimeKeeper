...
// GET /api/meds — all medications with their schedule slots.
export async function GET() {
  if (!(await isAdminRequest())) return unauthorized();
  const list = await store.meds();
  return json(list.map((m) => ({ id: m.id, name: m.name, dose: m.dose, active: m.active, slots: m.slots })));
}

// POST /api/meds?name=&dose=&slots=<json>&id= — create or update a medication.
export async function POST(req: NextRequest) {
  if (!(await isAdminRequest())) return unauthorized();
  const id = num(req, "id");
  const name = q(req, "name") ?? "";
  const dose = q(req, "dose") ?? "";
  const slots = parseSlots(q(req, "slots") ?? "[]");
  if (!name) return errJson("Medication name is required.");
  if (slots.length === 0) return errJson("Add at least one day and time.");
  await store.saveMed(id, name, dose, slots);
  return ok();
}
